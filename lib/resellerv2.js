...
const addPremiumUser = (userId, expired) => {
    let _dir = JSON.parse(fs.readFileSync('./lib/database/resellerv2.json'));
    ...
    _dir.push(obj)
    fs.writeFileSync('./lib/database/resellerv2.json', JSON.stringify(_dir, null, 2))
}
...
`
            		const contentText = {
            			text: teks,
            			contextInfo: {
            				mentionedJid: client.ments(teks),
            				externalAdReply: {
        					title: `RESELLER EXPIRED 🍀`,
        					previewType: "PHOTO",
        					thumbnailUrl: `https://files.catbox.moe/ovzzji.jpg`,
        					sourceUrl: "-"
            				}
            			}
            		};
            		client.sendMessage(_dir[position].id, contentText)
