...
const date = moment.tz('Asia/Jakarta').format(`DD MMM yyyy`)
  async function createUser(email, username, password) {
    		const response = await axios.post(global.domain + '/api/application/users', {
    			email,
    			username,
    			first_name: username,
    			last_name: username,
    	...
    	async function createServer(name, userId, startup_cmd, memo, cpu, disk) {
    		const response = await axios.post(global.domain + `/api/application/servers`, {
    			name,
    	...
    	async function getEggStartupCommand() {
    		const response = await axios.get(global.domain + `/api/application/nests/${global.nets}/eggs/${global.eggs}`, {
    			headers: {
    				"Accept": "application/json",
    				"Content-Type": "application/json",
    				"Authorization": `Bearer ${global.apiuser}`
        ...
        try {
            const response = await axios.post(global.domain + `/api/client/servers/${srv}/power`, {
                signal: action
            }, {
                headers: {
                    "Accept": "application/json",
    ...
    try {
        let response = await fetch(global.domain + "/api/application/servers/" + srv, {
            method: "DELETE",
            headers: {
                "Accept": "application/json",
                "Content-Type": "application/json",
...
async function deleteUser(usr) {
    try {
        let response = await fetch(global.domain + "/api/application/users/" + usr, {
            method: "DELETE",
            headers: {
                "Accept": "application/json",
                "Content-Type": "application/json",
